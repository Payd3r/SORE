                                                    ...
                                                    <div className="w-32 h-32 lg:w-48 lg:h-48 bg-gradient-to-br from-blue-500/10 to-blue-600/10 dark:from-blue-900/20 dark:to-blue-800/20 rounded-2xl flex items-center justify-center text-2xl lg:text-3xl text-blue-500 dark:text-blue-400 font-medium shadow-sm">
                                                        {
                                                            userInfoData.profile_picture_url
                                                                ? <img src={getImageUrl(userInfoData.profile_picture_url)} alt={userInfoData.name} className="w-full h-full rounded-2xl object-cover" />
                                                                : userInfoData.name[0]}
                                                    </div>
                                                    <div className="flex-1 text-center md:text-left">
                                                        <h3 className="text-xl lg:text-2xl font-medium text-gray-800 dark:text-white mb-2">{userInfoData.name}</h3>
                                                        ...
                                                        <div className="space-y-2 mb-6 lg:mb-8">
                                                            <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-sm lg:text-base">
                                                                ...
                                                                <span>Tema preferito: {userInfoData.theme_preference === 'system' ? 'Sistema' : userInfoData.theme_preference === 'light' ? 'Chiaro' : 'Scuro'}</span>
                                                            </div>
                                                            <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-sm lg:text-base">
                                                                <svg className="w-4 h-4 lg:w-5 lg:h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                <div className="space-y-3 lg:space-y-4">
                                                    {coupleInfo.membri && coupleInfo.membri.map((membro, index) => (
                                                        <div key={index} className="group hover:scale-[1.02] transition-all duration-200">
                                                            <div className={`flex items-center gap-3 lg:gap-4 ${membro.id === userInfoData.id ? 'bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800' : 'bg-gray-50 dark:bg-gray-700/50'} p-4 lg:p-5 rounded-xl`}>
                                                                <div className={`w-12 h-12 lg:w-16 lg:h-16 ${membro.id === userInfoData.id ? 'bg-gradient-to-br from-blue-500 to-blue-600' : 'bg-gradient-to-br from-blue-500/10 to-blue-600/10 dark:from-blue-500 dark:to-blue-600'} rounded-xl flex items-center justify-center text-xl lg:text-2xl ${membro.id === userInfoData.id ? 'text-white' : 'text-blue-500 dark:text-white'} font-medium`}>
                                                                    {membro.name[0]}
                                                                </div>
                                                                <div>
                                                                    <div className={`text-base lg:text-lg font-medium ${membro.id === userInfoData.id ? 'text-blue-600 dark:text-blue-400' : 'text-gray-800 dark:text-white'}`}>{membro.name}</div>
                                                                    <div className="text-xs lg:text-sm text-gray-500 dark:text-gray-400">{membro.email}</div>
                                                                    {membro.id === userInfoData.id && (
