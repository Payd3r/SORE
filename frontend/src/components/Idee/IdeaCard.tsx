...
export default function IdeaCard({ idea: initialIdea, onCheckChange, onClick }: IdeaCardProps) {
  ...
  return (
    <>
      <div 
        className={`group relative bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border-2 ${colors.border} min-h-[120px] touch-manipulation`}
        onClick={handleCardClick}
        role="button"
        tabIndex={0}
      >
        {/* Badge tipo */}
        <div className={`absolute top-3 left-3 px-2 py-1 rounded-full text-xs font-medium ${colors.badge} pointer-events-none`}>
        ...
        {/* Checkbox */}
        <div 
          className="absolute top-2 right-2 z-10 p-2 -m-2"
          ...
          <div className="flex items-start gap-3">
            <div className="flex-1">
              ...
              <div className="mt-2 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <span className="pointer-events-none">{idea.created_by_name}</span>
              </div>
            </div>
          </div>
        </div>
