...
const CardRicordoMobile = memo(({ memory, onClick, isActive }: MemoryCardMobileProps) => {
        ...
        <div className="grid grid-cols-3 grid-rows-1 gap-0.5 h-full">
          {immagini.length > 0 && (
            <div className="col-span-2 row-span-1 relative">
              ...
              <div className="absolute bottom-0 left-0 right-0 h-[30%] bg-gradient-to-t from-black/70 to-transparent"></div>
  ...
  const SpotifyPreview = () => {
    ...
    if (spotifyData) {
      return (
        <div className="flex items-center py-1.5 px-3 bg-black/50 backdrop-blur-lg">
  ...
  return (
    ...
    >
      ...
      <div className={`${isViaggio && memory.song ? 'h-44' : typeStyle.imageHeight} w-full relative overflow-hidden`}>
        {getImagesLayout()}
      ...
      {/* Player Spotify in una sezione dedicata sotto l'immagine (solo per viaggi) */}
