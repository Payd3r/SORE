  ...
  return (
    <div className="grid grid-cols-2 gap-2" aria-hidden="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={`memory-skeleton-${index}`} className="rounded-2xl bg-[var(--skeleton-card-bg)] p-2">
...
export function SkeletonIdeasListMobile({ count = 5 }: SkeletonListProps) {
  return (
    <div className="space-y-2" aria-hidden="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={`idea-skeleton-${index}`} className="rounded-2xl bg-[var(--skeleton-card-bg)] p-4">
...
export function SkeletonNotificationItemMobile({ count = 6 }: SkeletonListProps) {
  return (
    <div className="space-y-3" aria-hidden="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={`notification-skeleton-${index}`} className="rounded-2xl bg-[var(--skeleton-card-bg)] p-4">
...
export function SkeletonProfileHeaderMobile() {
  return (
    <div className="rounded-2xl bg-[var(--skeleton-card-bg)] p-6" aria-hidden="true">
    ...
    <div className="space-y-4" aria-hidden="true">
      <div className="grid grid-cols-2 gap-3">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={`stats-card-${index}`} className="rounded-2xl bg-[var(--skeleton-card-bg)] p-4">
            <Skeleton className="h-3 w-1/2" />
            <Skeleton className="mt-3 h-7 w-1/3" />
          </div>
        ))}
      </div>
      <div className="rounded-2xl bg-[var(--skeleton-card-bg)] p-5">
...
export function SkeletonMemoryDetailMobile() {
  return (
    <div className="fixed inset-0 z-[100000] bg-[var(--bg-page)]" aria-hidden="true">
