...
export default function MobileUploadStatus() {
  ...
  // Rimuove automaticamente i file completati dopo un certo periodo di tempo
  useEffect(() => {
    // Verifica se tutti i file sono stati completati
    const allCompleted = Object.values(uploadingFiles).every(
      file => file.status === 'completed'
    );
    
    // Se tutti i file sono completati, imposta un timer per rimuoverli
    if (allCompleted && Object.keys(uploadingFiles).length > 0) {
      const timer = setTimeout(() => {
        setUploadingFiles({});
      }, 3000); // 3 secondi dopo il completamento di tutti i file
  ...
  const uploadButton = (
    <button 
      ...
      className={`fixed bottom-24 right-4 z-50 flex items-center justify-center w-12 h-12 rounded-full shadow-lg ${
  ...
  const modalContent = isModalOpen && createPortal(
    <div className="fixed inset-0 z-[9999] overflow-y-auto bg-black bg-opacity-75 flex flex-col">
      <div className="bg-white dark:bg-gray-900 rounded-t-2xl mt-auto max-h-[80vh] flex flex-col pb-6">
        {/* Header */}
        ...
        <div className="border-t border-gray-200 dark:border-gray-800 p-4">
          <div className="flex justify-between gap-3">
            ...
            >
              {allCompleted ? 'Fatto' : 'Rimuovi tutti'}
            </button>            
          </div>
        </div>
      </div>
